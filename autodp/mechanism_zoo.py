...
class GaussianMechanism(Mechanism):
    def __init__(self, sigma, coeff=None, name='Gaussian',
                 ...
                 use_fDP_based_RDP_to_approxDP_conversion=False, CDF_off=True):
        ...
        if not CDF_off:
            cdf_approx = False
            char_approx = False#if there is no closed-form expression of characteristic function, then we use approximation

            if not cdf_approx and not char_approx:
                """
            ...
            elif  cdf_approx and char_approx:
                """
                use analytical way to compute phi 
                """
                phi = lambda x: phi_bank.phi_gaussian({'sigma': sigma},x,coeff=coeff)
                #phi = lambda x: phi_bank.phi_gaussian_ana({'sigma': sigma},x,coeff = coeff)
...
class SubSampleGaussian(Mechanism):
    def __init__(self, sigma,gamma,coeff=1, name='Subsample_Gaussian',
        ...
        if not CDF_off:
            cdf_approx = True
            char_approx = True  # if there is no closed-form expression of characteristic function, then we use approximation

            if  cdf_approx and char_approx:
                """
                use analytical way to compute phi 
                """
